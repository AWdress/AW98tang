...
### ⭐ 在线更新与版本管理（全面升级）
- **🔄 Web在线更新：ZIP方式**（无需代理/凭据）
  - 公开仓库走镜像（ghproxy/kgithub/fgit）
  - 私有仓库支持 `GITHUB_TOKEN` 直接 zipball 下载
- **📦 版本信息模块**：侧边栏自动显示“当前/最新版本 + Commit”
- **🕒 静默检查**：页面加载即检查，后续每10分钟自动检查一次
- **⚠️ 启动更新开关**：`STARTUP_AUTO_UPDATE=true` 才在启动时尝试更新（ZIP优先）
- **✅ 自动重启**：在线更新成功后3秒自动重启容器并自动刷新页面
